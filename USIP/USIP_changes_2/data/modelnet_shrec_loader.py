...
def make_dataset_modelnet40_10k(root, mode, opt, part):
    ...
    if 'train' == mode:
        path = os.path.join(root,mode,folder)
        files = [ob for ob in os.listdir(path) if ob.endswith(".npy")]
        nsamples = len(files)
        print("Automatically detected", nsamples, "training files!")
    elif 'test' == mode:
        path = os.path.join(root,mode,folder)
        files = [ob for ob in os.listdir(path) if ob.endswith(".npy")]
        nsamples = len(files)
        print("Automatically detected", nsamples, "test files!")
    else:
...
class ModelNet_Shrec_Loader(data.Dataset):
    def __init__(self, root, mode, opt):
        ...
        if self.opt.dataset == 'modelnet':
            ### ulsteger:  make two datasets instead of one, containing the two different fragments of the pairs
    ...
    def get_instance_unaugmented_np(self, index, part):
        ### ulsteger: load the respective fragment of the pair, depending on the parameter "part"
        if self.opt.dataset == 'modelnet':
            if part == 1:
