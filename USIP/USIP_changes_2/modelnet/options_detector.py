...
class Options():
    ...
    def initialize(self):
        ...
        ### ulsteger: CHANGE path to location of the training pairs
        self.parser.add_argument('--dataroot', default=config.dataroot, help='path to images & laser point clouds')
        ...
        ### ulsteger: CHANGE path to location, where the trained models should be saved
        self.parser.add_argument('--checkpoints_dir', type=str, default=config.model_save_path, help='models are saved here')

        self.parser.add_argument('--batch_size', type=int, default=config.batch_size, help='input batch size')
        ### ulsteger: reduced number of input points from 5000 to 1000, since our datasets are smaller
        self.parser.add_argument('--input_pc_num', type=int, default=config.number_of_pointclouds, help='# of input points')
        ...
        self.parser.add_argument('--node_num', type=int, default=config.node_num, help='som node number')
        self.parser.add_argument('--k', type=int, default=config.knn_k, help='k nearest neighbor')
        ...
        self.parser.add_argument('--keypoint_outlier_thre', type=float, default=0.3, help='Threshold of distance between cloesest keypoint pairs, large distance is considered to be mis-matched.')

        ### ulsteger: changed alpha from 0.5 to 5, to force the keypoints to be closer to the surface
        self.parser.add_argument('--keypoint_on_pc_alpha', type=float, default=config.kp_alpha, help='weight of keypoint_on_pc loss, default 0.5.')
