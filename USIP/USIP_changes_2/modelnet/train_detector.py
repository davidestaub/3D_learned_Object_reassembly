...
import config 
sys.path.append(config.usip_root)
...
### ulsteger: added path to the pretrained model, trained on modelnet
detector_model_path = config.pretrained_model_path

### ulsteger: copied this function from evaluation/save_keypoints.py to load petrained model
...
if __name__=='__main__':
    ...
    ### ulsteger: load pretrained model, trained on modelnet
    model.detector.load_state_dict(
      model_state_dict_convert_auto(
        torch.load(
          detector_model_path,
          map_location='cpu'), opt.gpu_ids))

    # wandb setup
    wandb.login(key="13be45bcff4cb1b250c86080f4b3e7ca5cfd29c2")
    wandb.init(project="3dvision", entity="matvogel")
    wandb.config = {
    ...
    for epoch in range(config.epochs):
        ...
        # logging wandb values
        wandb.log({"sigma_mean": sigma_mean})
        wandb.log({"sigma_std": sigma_std})
        wandb.log({"sigma_max": sigma_max})
        wandb.log({"sigma_min": sigma_min})
        ...
        if epoch >= 0 and epoch%1==0:
            ...
            # print loss average to wandb
            wandb.log({"loss_avg": model.test_loss_average.item()})
